    ...
    if len(content) > 1000:
        ...
        try:
            ...
            if 500 < len(condensed) <= 1000:
                ...
                print(f"  ⚠️ 精简结果 {len(condensed)} 字不在合理范围，保持原文")
        except Exception as e:
            print(f"  ⚠️ LLM精简失败: {e}，保持原文")
...
def generate_content(state: dict) -> dict:
    ...
    for provider in _LLM_PROVIDERS:
        ...
        try:
            ...
            # 平台限制：标题20字、正文1000字，超出则优雅处理
            title, content = _condense_if_needed(result.title, result.content, llm)
